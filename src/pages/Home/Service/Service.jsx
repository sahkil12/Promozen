...
const Service = () => {
  ...
  return (
    <section className="relative py-24 bg-secondary overflow-hidden border-t border-base-300/5 inter">
      ...
      <div className="relative z-10 w-full xl:max-w-[75%] mx-auto px-6">
        {/* Header Section */}
        <div className="mb-16">
        ...
        <div
          ...
          {services.map((service, index) => (
            <motion.div
              ...
              className="group relative p-8 rounded-2xl bg-[#121212] border border-primary/10 hover:border-primary/45 transition-all duration-300 flex flex-col h-full hover:drop-shadow-[0_0_20px_rgba(242,201,76,0.1)]"
            >
              <Link to={service.link}>
                {/* Icon Container */}
                <div className="w-14 h-14 rounded-2xl border border-primary/15 flex items-center justify-center text-primary text-2xl bg-primary/10 mb-8 group-hover:bg-primary/15 group-hover:drop-shadow-[0_0_20px_rgba(242,201,76,0.5)] group-hover:border-primary/20 transition-all duration-300">
                  {service.icon}
                </div>

                {/* Content */}
                <h3 className="text-xl font-semibold poppins text-white mb-4 group-hover:text-primary transition-colors">
                  {service.title}
                </h3>
                ...
                <a
                  href={service.link}
                  className="inline-flex opacity-0 group-hover:opacity-100 translate-y-full group-hover:translate-y-0 duration-300 overflow-hidden items-center gap-2 text-primary text-sm font-bold hover:gap-3 transition-all"
                >
                  Learn More <FiArrowRight />
                </a>
                {/* Hover Glow Effect */}
                <div className="absolute inset-0 rounded-2xl bg-primary/5 opacity-0 group-hover:opacity-15 blur-2xl transition-opacity -z-10"></div>
              </Link>
            </motion.div>
          ))}
        </div>
